...
class MCMC():
    def __init__(self, simtime, samples, communities, core_data, core_depths,timestep,filename, xmlinput, sedsim, sedlim, flowsim, flowlim, max_a, max_m, vis):
        ...
        self.initial_sed = []
        self.initial_flow = []
        self.step_m =0.02# 0.002 #<1%
        self.step_a =0.02# 0.002  #<1%
        self.step_sed =0.0001# 0.0001 #2%
        self.step_flow =0.006# 0.0015 #<1%
    ...
    def sampler(self):
        ...
        if self.sedsim == True:
            for s in range(self.communities):
                sed1[s] = pos_sed1[0,s] = np.random.uniform(0.,0.005)
                sed2[s] = pos_sed2[0,s] = np.random.uniform(sed1[s],0.005)
                sed3[s] = pos_sed3[0,s] = np.random.uniform(sed2[s],0.005)
                sed4[s] = pos_sed4[0,s] = np.random.uniform(sed3[s],0.005)
        ...
        if self.flowsim == True:
            for s in range(self.communities):
                #     relaxed constraints 
                flow1[s] = pos_flow1[0,s] = np.random.uniform(0.,0.3)
                flow2[s] = pos_flow2[0,s] = np.random.uniform(flow1[s],0.3)
                flow3[s] = pos_flow3[0,s] = np.random.uniform(flow2[s],0.3)
                flow4[s] = pos_flow4[0,s] = np.random.uniform(flow3[s],0.3)
